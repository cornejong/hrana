...
type CursorEntry struct {
	...
	// step_end
	AffectedRowCount *uint32 `json:"affected_row_count,omitempty"`
...
type HelloErrorMsg struct {
	Type  string `json:"type"` // "hello_error"
...
type ResponseOkMsg struct {
	Type      string `json:"type"` // "response_ok"
	RequestID int32  `json:"request_id"`
	Response  any    `json:"response"`
}

type ResponseErrorMsg struct {
	Type      string `json:"type"` // "response_error"
	RequestID int32  `json:"request_id"`
	Error     Error  `json:"error"`
}
